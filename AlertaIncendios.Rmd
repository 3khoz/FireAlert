...
mySIDCO <- readOGR("data/sidco_08022018t.shp", encoding = "UTF-8")
...
leaflet(width = 800, height = 600) %>% addProviderTiles(providers$Esri.WorldImagery) %>%
    ...
    # setView(lng = min(spa$longitude), lat = min(spa$latitude), zoom = 7) %>%
     ...
     addPolygons(data = mySN, fill = TRUE, stroke = TRUE, color = "#0B6803", highlightOptions = highlightOptions(color = "white", weight = 2),
