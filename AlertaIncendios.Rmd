...
#### El día de hoy se han detectado **`r format(focos)`** focos de incendios que se encuentra a menos de 5 km de **`r format(focos)`** sitios de conservación distribuidos en **`r format(focos)`** regiones
...
| Foco | Protección | Nombre |Región | Distancia al foco (km) | Aspectos relevantes|
|:---------------|:-------------:|---------------:|
| `r format(focos)`| está alineada | a la derecha |
...
leaflet() %>% addProviderTiles(providers$Esri.WorldImagery) %>%
  ...
  addCircles(data = sMODIS[,2:3],radius = 5000 ,fill = F, stroke = TRUE, color = "#F70B81", 
