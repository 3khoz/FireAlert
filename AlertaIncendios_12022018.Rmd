...
mySC <- readOGR("data/Proteccion2.shp", encoding = "UTF-8")
...
myReg <- readOGR("data/Regiones.shp", encoding = "UTF-8")
...
Situni<-unique(resumen$Sitio)
tablex<-subset(mySC,mySC$NAME==Situni)
...
leaflet(width = 800, height = 600) %>% addProviderTiles(providers$Esri.WorldImagery) %>%
    ...
    # setView(lng = min(spa$longitude), lat = min(spa$latitude), zoom = 7) %>%
     ...
     addPolygons(data = myReg, fill = F, stroke = TRUE, color = "#000000",dashArray = "3",highlightOptions = highlightOptions(color = "white", weight = 2), popup = paste("Región: "," : ", as.character(myReg$NOMBRE)), group = "Límite Regional") %>%
     # addPolygons(data = myRCP, fill = F, stroke = TRUE, color = "#000000",dashArray = "3",highlightOptions = highlightOptions(color = "white", weight = 2),popup = paste("Comuna: "," : ", as.character(myRCP$Comuna)), group = "Límite Comunal") %>%
    ...
    addLegend("bottomright", colors = c("#FF0000","#FFB233","#000000","#12D502","#FFFFFF","#FCEE07","#E2A904","#049BE2","#E204D8"), labels = c("SIDCO radio: 5 km","Puntos de calor","Límites Administrativos","SNASPE","SN","BNP","SP","ICP","AAVC")) %>%
    
  addLayersControl(overlayGroups = c("SIDCO","Punto de calor VIIRS 48h","Punto de calor MODIS 48h","Límite Regional","SNASPE","SN","BNP","SP","ICP","AAVC"),options = layersControlOptions(collapsed = FALSE))%>%
