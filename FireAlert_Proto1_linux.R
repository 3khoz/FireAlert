...
setwd("/mnt/4A34382F34381FFD/GEODATABASE/VECTOR/Chile_utm19/chile_utm19/chile_continental_utm19/")
...
setwd("/mnt/4A34382F34381FFD/GEODATABASE/VECTOR/SNASPE_simplify/")
...
d2<-dist.modis[dist.modis[,15:115]<10000,]

library(dplyr)
filter(dist.modis, dist.modis[,15:115] <= 5000)
...
leaflet() %>% addTiles() %>%   
  addCircles(data = dist.modis[,2:3], fill = T, stroke = TRUE, color = "#FF0000", 
             popup = paste0("Fecha: ", as.character(dist.modis$acq_date)), group = "HotSpot MODIS") %>% 
  addCircles(data = st_vii@coords, fill = T, stroke = TRUE, color = "#FF8000", 
             popup = paste0("Fecha: ", as.character(dist.modis$acq_date)), group = "HotSpot Viirs") %>%
  addPolygons(data = myASP, fill = TRUE, stroke = TRUE, color = "#008000", 
              popup = paste0("Unidad: ", as.character(myASP@data[,3])), group = "ASP") %>% 
  # add a legend
  addLegend("bottomright", colors = c("#FF0000","#FF8000", "#008000"), labels = c("HotSpot MODIS","HotSpot Viirs", "ASP")) %>%   
  # add layers control
  addLayersControl(
