...
mySC <- geojsonio::geojson_read("data/Proteccion3.json", what = "sp", encoding = "UTF-8")
projection(mySC)<- CRS("+proj=longlat +datum=WGS84")
mySC_utm<-spTransform(mySC, CRS("+proj=utm +south +zone=19 +datum=WGS84"))

## Especies monumento
NOAL<-subset(mySC,mySC@data$NAME=="Nothofagus alessandrii - ruil (EN)")
PIPU<-subset(mySC,mySC@data$NAME=="Pitavia punctata - pitao (EN)")
GOKE<-subset(mySC,mySC@data$NAME=="Gomortega keule - queule (EN)")
FICU<-subset(mySC,mySC@data$NAME=="Fitzroya cupressoides - alerce, lahuÃ¡n (EN)")
ARAR<-subset(mySC,mySC@data$NAME=="Araucaria araucana - araucaria, pehuÃ©n (VU)")
BEBE<-subset(mySC,mySC@data$NAME=="Beilschmiedia berteroana - belloto del sur (EN)")
BEMI<-subset(mySC,mySC@data$NAME=="Beilschmiedia miersii - belloto del norte (VU)")
...
bicho<-cbind(bichos[,4:5],bichos[,8:10],orden=bichos[,20])
...
compi<-cbind(Incendio=resumen[1],resumen[5:8],Estado=name1,resumen[11],resumen[10],resumen[12])
...
coln<-c("Foco","Región","Provincia","Comuna","Estado actual","Tipo de protección","Nombre","Distancia al foco (km)")
...
**EMN**    = Especies Monumento Natural.
...
tasp<-iconv.data.frame(as.data.frame(as.character(myASP@data[,4])),from="UTF8",to="latin1")
tsn<-iconv.data.frame(as.data.frame(as.character(mySN@data[,4])),from="UTF8",to="latin1")
tbnp<-iconv.data.frame(as.data.frame(as.character(myBNP@data[,4])),from="UTF8",to="latin1")
tsp<-iconv.data.frame(as.data.frame(as.character(mySP@data[,4])),from="UTF8",to="latin1")
ticp<-iconv.data.frame(as.data.frame(as.character(myICP@data[,4])),from="UTF8",to="latin1")
taavc<-iconv.data.frame(as.data.frame(as.character(myAAVC@data[,4])),from="UTF8",to="latin1")
tnoal<-iconv.data.frame(as.data.frame(as.character(NOAL@data[,4])),from="UTF8",to="latin1")
tpipu<-iconv.data.frame(as.data.frame(as.character(PIPU@data[,4])),from="UTF8",to="latin1")
tgoke<-iconv.data.frame(as.data.frame(as.character(GOKE@data[,4])),from="UTF8",to="latin1")
tficu<-iconv.data.frame(as.data.frame(as.character(FICU@data[,4])),from="UTF8",to="latin1")
tarar<-iconv.data.frame(as.data.frame(as.character(ARAR@data[,4])),from="UTF8",to="latin1")
tbebe<-iconv.data.frame(as.data.frame(as.character(BEBE@data[,4])),from="UTF8",to="latin1")
tbemi<-iconv.data.frame(as.data.frame(as.character(BEMI@data[,4])),from="UTF8",to="latin1")


# nombres
nasp<-iconv.data.frame(as.data.frame(as.character(myASP@data[,5])),from="UTF8",to="latin1")
nsn<-iconv.data.frame(as.data.frame(as.character(mySN@data[,5])),from="UTF8",to="latin1")
nbnp<-iconv.data.frame(as.data.frame(as.character(myBNP@data[,5])),from="UTF8",to="latin1")
nsp<-iconv.data.frame(as.data.frame(as.character(mySP@data[,5])),from="UTF8",to="latin1")
nicp<-iconv.data.frame(as.data.frame(as.character(myICP@data[,5])),from="UTF8",to="latin1")
naavc<-iconv.data.frame(as.data.frame(as.character(myAAVC@data[,5])),from="UTF8",to="latin1")
nnoal<-iconv.data.frame(as.data.frame(as.character(NOAL@data[,5])),from="UTF8",to="latin1")
npipu<-iconv.data.frame(as.data.frame(as.character(PIPU@data[,5])),from="UTF8",to="latin1")
ngoke<-iconv.data.frame(as.data.frame(as.character(GOKE@data[,5])),from="UTF8",to="latin1")
nficu<-iconv.data.frame(as.data.frame(as.character(FICU@data[,5])),from="UTF8",to="latin1")
narar<-iconv.data.frame(as.data.frame(as.character(ARAR@data[,5])),from="UTF8",to="latin1")
nbebe<-iconv.data.frame(as.data.frame(as.character(BEBE@data[,5])),from="UTF8",to="latin1")
nbemi<-iconv.data.frame(as.data.frame(as.character(BEMI@data[,5])),from="UTF8",to="latin1")



leaflet(width = 820, height = 800) %>% 
     ...
     addPolygons(data = NOAL, fill = TRUE, stroke = TRUE, color = "#0008FF", highlightOptions = highlightOptions(color = "white", weight = 2),
                popup = paste(as.character(tnoal[,1])," : ", as.character(nnoal[,1])), group = "Ruil") %>%
     addPolygons(data = PIPU, fill = TRUE, stroke = TRUE, color = "#0008FF", highlightOptions = highlightOptions(color = "white", weight = 2),
                popup = paste(as.character(tpipu[,1])," : ", as.character(npipu[,1])), group = "Pitao") %>%
     addPolygons(data = GOKE, fill = TRUE, stroke = TRUE, color = "#0008FF", highlightOptions = highlightOptions(color = "white", weight = 2),
                popup = paste(as.character(tgoke[,1])," : ", as.character(ngoke[,1])), group = "Keule") %>%
     addPolygons(data = FICU, fill = TRUE, stroke = TRUE, color = "#0008FF", highlightOptions = highlightOptions(color = "white", weight = 2),
                popup = paste(as.character(tficu[,1])," : ", as.character(nficu[,1])), group = "Alerce") %>%
     addPolygons(data = ARAR, fill = TRUE, stroke = TRUE, color = "#0008FF", highlightOptions = highlightOptions(color = "white", weight = 2),
                popup = paste(as.character(tarar[,1])," : ", as.character(narar[,1])), group = "Araucaria") %>%
     addPolygons(data = BEBE, fill = TRUE, stroke = TRUE, color = "#0008FF", highlightOptions = highlightOptions(color = "white", weight = 2),
                popup = paste(as.character(tbebe[,1])," : ", as.character(nbebe[,1])), group = "Belloto del sur") %>%
     addPolygons(data = BEMI, fill = TRUE, stroke = TRUE, color = "#0008FF", highlightOptions = highlightOptions(color = "white", weight = 2),
                popup = paste(as.character(tbemi[,1])," : ", as.character(nbemi[,1])), group = "Belloto del norte") %>%

    addLegend("bottomright", colors = c("#FF0000","#FFB233","#12D502","#FFFFFF","#FCEE07","#E2A904","#049BE2","#E204D8","#0008FF"), labels = c("SIDCO radio: 5 km","Puntos de calor","SNASPE","SN","BNP","SP","ICP","AAVC","EMN")) %>%
    
    addLayersControl(baseGroups = c("Satélite", "Temático"),
      
    overlayGroups = c("SIDCO","Punto de calor VIIRS 48h","Punto de calor MODIS 48h","SNASPE","SN","BNP","SP","ICP","AAVC","Ruil","Pitao","Keule","Alerce","Araucaria","Belloto del sur","Belloto del norte"),
                   options = layersControlOptions(collapsed = FALSE))%>%
    hideGroup(c("Límite Regional","Punto de calor VIIRS 48h","Punto de calor MODIS 48h","SNASPE","SN","BNP","SP","ICP","AAVC","Ruil","Pitao","Keule","Alerce","Araucaria","Belloto del sur","Belloto del norte"))
